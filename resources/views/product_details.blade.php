                ...
                @if ($galleryImages->count() > 1)
                    <div class="d-flex flex-wrap justify-content-center mt-3" style="gap:12px;">
                        @foreach ($galleryImages as $galleryImage)
                            <button type="button"
                                class="btn btn-light p-0 border gallery-thumb"
                        ...
                        <li class="d-flex justify-content-between align-items-center py-3">
                            ...
                            <span id="product-stock-badge"
                                ...
                                @if ($product->stock_quantity > 0)
                                    {{ __('store.pages.product_details.in_stock') }} ({{ __('store.pages.product_details.quantity_left', ['count' => $product->stock_quantity]) }})
                                @else
                    ...
                    @if ($product->has_variants)
                        <div class="mb-4">
                            ...
                            <div class="d-flex flex-wrap" id="variant-colors" style="gap:10px;">
                                @foreach ($availableColors as $color)
                                    <button type="button"
                                        class="btn btn-outline-dark variant-color-btn"
                                        data-color-id="{{ $color->id }}"
                                        data-color-name="{{ $color->name }}"
                                        style="border-radius:999px;">
                                        @if ($color->hex_code)
                                            <span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:{{ $color->hex_code }};margin-right:8px;"></span>
                                        @endif
                                        {{ $color->name }}
                                    </button>
                                @endforeach
                        ...
                        <div class="mb-4">
                            ...
                            <div class="d-flex flex-wrap" id="variant-sizes" style="gap:10px;">
                                <span class="text-muted">{{ app()->getLocale() === 'ar' ? 'اختر لونًا أولًا' : 'Select a color first' }}</span>
                            </div>
                        </div>
            ...
            const renderSizes = (colorId) => {
                ...
                colorVariants.forEach((variant) => {
                    ...
                    button.addEventListener('click', () => {
                        ...
                        stockBadge.className = 'badge badge-success';
                        stockBadge.textContent = @json(__('store.pages.product_details.in_stock'));
                        summaryText.textContent = `${variant.color_name} / ${variant.size_name}${variant.sku ? ' · ' + variant.sku : ''} · ${variant.stock} @json(__('store.common.left'))`;
                    });
